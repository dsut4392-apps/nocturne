...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        // Generate TypeScript API client using NSwag
        // Only generate in development mode; in publish mode, assume it's already generated by the CI/CD pipeline
        if (!builder.ExecutionContext.IsPublishMode)
        {
            var apiProjectPath = Path.Combine(solutionRoot, "src", "API", "Nocturne.API");
            Console.WriteLine("[Aspire] Generating TypeScript API client with NSwag...");

            var nswagProcess = new System.Diagnostics.Process
            {
                StartInfo = new System.Diagnostics.ProcessStartInfo
                {
                    FileName = "dotnet",
                    Arguments = "build -t:GenerateClient",
            ...
            var nswagStdoutTask = nswagProcess.StandardOutput.ReadToEndAsync();
            var nswagStderrTask = nswagProcess.StandardError.ReadToEndAsync();

            await nswagProcess.WaitForExitAsync();

            var nswagStdout = await nswagStdoutTask;
            var nswagStderr = await nswagStderrTask;

            if (nswagProcess.ExitCode != 0)
            {
                // Log warning but don't fail - NSwag generation is not critical for startup
                Console.WriteLine(
                    $"[Aspire] Warning: NSwag client generation failed (exit code {nswagProcess.ExitCode})"
                );
                Console.WriteLine($"[Aspire] NSwag stderr: {nswagStderr}");
            }
            else
            {
                Console.WriteLine("[Aspire] TypeScript API client generated successfully");
            }
        }
        else
        {
            Console.WriteLine(
                "[Aspire] Skipping NSwag client generation (publish mode - assuming pre-generated)"
            );
        }
