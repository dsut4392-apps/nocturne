<script lang="ts">
  ...
  import { Chart, Axis, Svg, Spline, Group, Rule } from "layerchart";
  ...
  const filteredTreatments = $derived(
    treatments.filter((t) => {
      const treatmentTime = t.mills ?? (t.created_at ? new Date(t.created_at).getTime() : 0);
      return treatmentTime >= displayDateRange.from.getTime() && 
  ...
  const bolusTreatments = $derived(
    filteredTreatments.filter((t) => 
      ...
      ['Meal Bolus', 'Correction Bolus', 'Snack Bolus', 'Bolus Wizard', 'Combo Bolus'].includes(t.eventType ?? '')
    )
  );
  ...
  const tempBasalTreatments = $derived(
    filteredTreatments.filter((t) => 
      t.eventType === 'Temp Basal' && (t.rate !== undefined || t.percent !== undefined)
    )
  );
  ...
  const yMax = $derived(Math.min(400, Math.max(100, ...entries.map((e) => e.sgv ?? 0)) + 50));
  ...
  <CardContent class="h-96">
    ...
    >
      <Svg>
        ...
        <Axis placement="bottom" rule format={(v) => v instanceof Date ? v.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : v} />

        <!-- Target range threshold lines using Rule -->
        <Rule y={chartConfig.high.threshold} class="stroke-destructive/60 [stroke-dasharray:4,4]" />
        <Rule y={chartConfig.low.threshold} class="stroke-warning/60 [stroke-dasharray:4,4]" />
        ...
        <Group class="temp-basals">
          {#each tempBasalTreatments as treatment}
            ...
            {@const endX = getXPercent(getTreatmentTime(treatment) + duration * 60 * 1000)}
            <line
              x1="{startX}%"
              x2="{endX}%"
              y1="2%"
            ...
            >
              {treatment.rate !== undefined ? `${treatment.rate.toFixed(1)}` : `${treatment.percent}%`}
            </text>
          {/each}
        </Group>
