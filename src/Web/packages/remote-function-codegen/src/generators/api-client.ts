 ...
 *
 * Note: The current api-client.ts has custom property names that don't follow
 * a simple naming pattern. This generator produces a basic version that may
 * need manual adjustments to match existing property names.
 */
...
// Source: openapi.json
//
// Note: Property names are derived from NSwag client class names.
...
function tagToClientInfo(tag: string): ClientInfo {
  // Handle versioned tags like "V4 Trackers" -> "Trackers"
  const cleaned = tag.replace(/^V\d+\s*/i, '');

  // NSwag naming: Tag "Trackers" -> class "TrackersClient"
  // We derive property name as camelCase of the cleaned tag
  const className = `${cleaned}Client`;

  // Convert to camelCase for property name
  const propertyName = cleaned.charAt(0).toLowerCase() + cleaned.slice(1);
